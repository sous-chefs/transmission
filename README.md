...
* `node["transmission"]["rpc_whitelist"]` - IPs with access to RPC, possible to use wildcards such as *, default `127.0.0.1`.
* `node["transmission"]["rpc_whitelist_enabled"]` - Whether the RPC whitelist is used, default `true`.
